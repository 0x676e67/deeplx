...
pub(crate) const PID_PATH: &str = "/var/run/deepl.pid";
#[cfg(target_family = "unix")]
pub(crate) const DEFAULT_STDOUT_PATH: &str = "/var/run/deepl.out";
#[cfg(target_family = "unix")]
pub(crate) const DEFAULT_STDERR_PATH: &str = "/var/run/deepl.err";
...
pub fn start(args: BootArgs) -> Result<()> {
    if let Some(pid) = get_pid() {
        println!("deeplis already running with pid: {}", pid);
        return Ok(());
    }
